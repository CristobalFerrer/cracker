...
class DeepCrackTrainer(nn.Module):
    def __init__(self, model):
        ...
        # -------------------- Loss --------------------- #
        # Match pos_weight device to model (CPU/GPU); avoids cuda-only crash on CPU.
        _dev = next(model.parameters()).device
        self.mask_loss = nn.BCEWithLogitsLoss(
            reduction='mean',
            pos_weight=torch.tensor([float(cfg.pos_pixel_weight)], device=_dev, dtype=torch.float32),
        )
